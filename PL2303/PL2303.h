...
static inline unsigned int tval2long( mach_timespec_t value )
{
   return (value.tv_sec * NSEC_PER_SEC) + value.tv_nsec;
}

static inline mach_timespec_t long2tval( unsigned int value )
{
    mach_timespec_t   tval;

    tval.tv_sec  = value / NSEC_PER_SEC;
    tval.tv_nsec = value % NSEC_PER_SEC;
    return tval;
}
...
typedef struct
{
    ...
    enum tXO_State       RXOstate;    /* Indicates our receive state.    */
    enum tXO_State       TXOstate;    /* Indicates our transmit state, if we have received any Flow Control. */
    ...
    mach_timespec_t   DataLatInterval;
    mach_timespec_t   CharLatInterval;
