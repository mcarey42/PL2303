...
bool nl_bjaelectronics_driver_PL2303::start(IOService *provider)
{
	...
	// Allocate the power management features.
    PMinit();
...
/****************************************************************************************************/
//
//      Method:     nl_bjaelectronics_driver_PL2303::setPowerState
//
//      Inputs:     whichState - what state we are transitioning to, whatDevice - the IOService device
//                  object
//
//      Outputs:    None
//
//      Desc:       Performs power management for the PL2303 device in sleep states, or wakeups.
//
/****************************************************************************************************/

IOReturn nl_bjaelectronics_driver_PL2303::setPowerState ( unsigned long whichState, IOService * whatDevice )
...
/****************************************************************************************************/
//
//      Method:     nl_bjaelectronics_driver_PL2303::initialPowerStateForDomainState
//
//      Inputs:     flags - flags for initial power state setup
//                  object
//
//      Outputs:    None
//
//      Desc:       Performs power management for the PL2303 device on device startup (once only).
//
/****************************************************************************************************/

unsigned long nl_bjaelectronics_driver_PL2303::initialPowerStateForDomainState(IOPMPowerFlags flags)
{
    
    DEBUG_IOLog(4, "%s(%p)::initialPowerStateForDomainState(%08x)", getName(), this, flags);
	...
	switch ( type )
    {
		case kIOMessageServiceIsTerminated:
			...
			if ( fSessions ){
				...
				DEBUG_IOLog(4,"%s(%p)::message - kIOMessageServiceIsTerminated would have sent KUNCUserNotificationDisplayNotice\n", getName(), this);

                /* This has been depricated since 10.4.x and is now removed -Mark.
		...
		case kIOUSBMessageHubResumePort:
			...
			if ( !fTerminate )        
			{
				...
				if ( !startSerial() )
				{
					fTerminate = true;
					DEBUG_IOLog(4,"%s(%p)::message - startSerial failed\n", getName(), this);
                    /* Depricated since 10.4.x, now removed.  -Mark.
				    KUNCUserNotificationDisplayNotice(
					    0,      // Timeout in seconds
					    0,      // Flags (for later usage)
					    "",     // iconPath (not supported yet)
